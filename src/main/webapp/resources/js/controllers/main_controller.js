...
App.
	controller('MainController',
			['$scope','$http','MainModelService','$location',
			function($scope, $http,MainModelService,$location) {
				...
				self.getSessionData=function(){
			    	...
			    	.then(
			                function(response){
			                     ...
			                     self.logedUser=response.data;
			                     //TODO
			                     if(response.status==204){
			                    	 $location.url('login');
			                     }
			                     MainModelService.logedUserId=self.logedUser.idUser;
			                     self.chatActif=false;
			                     var categoriesToDelete=[];
			                     
			                     for(var categoryName in self.logedUser.userMenu){
			                    	 var categoryEntries=self.logedUser.userMenu[categoryName] ;
			                     ...
			                     for(var i=0;i<categoriesToDelete.length;i++){
			                    	...
			                    	delete self.logedUser.userMenu[cat];
			                     }
				...
				self.chatBox_popup=function(id){
					...
					if(!self.chatbox_in_front(id)){
						...
						self.newchatbox.idSender=self.logedUser.idUser;
						self.newchatbox.nameSender=self.logedUser.name;
